    ...
    <div class="filter">
        <table>
            ...
            <tr>
                ...
                <? endif ?>
            <? endif ?>
            <? if ($found_item_types_count > 0): ?>
            <tr>
                <td class="name"><?=$lang["found_item_type"]?>:</td>
                <td class="value"><?=html_selectbox_submit("fiid", $fount_item_types, $urlinfo["fiid"], $lang["(all_found_item_types)"], "style='width:100%'")?></td>
            </tr>
            <? endif ?>
            <tr>
                ...
                <td class="value" style="white-space: nowrap">
                    <fieldset style="border-style: hidden;margin: 0px;padding: 0px;">
                        ...
                        <table class="columns" id="columns" name="tshowcols">
                            ...
                            <tr>
                                <td><input type="checkbox" class="cols" name="col-found_item_type" id="col-found_item_type" onclick="toggleVis(this)" checked="checked"><label for="col-found_item_type"><?=$lang["found_item_type"]?></label></td>
...
while ($row = db_fetch_assoc($listdata)){
      echo "<form id=\"". $row["garmentusers_id"] ."\" enctype=\"multipart/form-data\" method=\"POST\" action=\"". $pi["filename_details"] ."\"><input type=\"hidden\" name=\"page\" value=\"details\"><input type=\"hidden\" name=\"id\" value=\"". $row["garmentusers_id"] ."\"><input type=\"hidden\" name=\"gosubmit\" value=\"true\"></form>";
        $rows .= "<tr class=\"list\" onClick=\"document.getElementById('". $row["garmentusers_id"] ."').submit();\">
            ...
            <td name=\"tcol-found_item_type\" id=\"tcol-found_item_type\" class=\"list\">". ((empty($row["found_item_type"])) ? "<span class=\"empty\">". $lang["unknown"] ."</span>" : $row["found_item_type"]) ."</td>  
            <td name=\"tcol-tag\" id=\"tcol-tag\" class=\"list\">". $row["tag"] ."</td>
            ...
            $rows .= "</td>";
            $rows .= "<td name=\"tcol-date\" id=\"tcol-date\" class=\"midlist\">". $row["date"] ."</td>";
            $rows .= "</tr>";
} ?>
...
<table class="list">
    <thead>
        <tr class="listtitle">
            ...
            <th name="tcol-found_item_type" id="tcol-found_item_type" class="list"><?=$sortlinks["found_item_type"]?></th>
