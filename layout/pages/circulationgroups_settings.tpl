...
<? if ($urlinfo["limit_total"] != 0){
    ...
    while ($row = db_fetch_assoc($listdata)){
        ...
        $rows .= "<tr class=\"list\" onClick=\"document.getElementById('". $row["id"] ."').submit();\">
            ...
            $rows .= "<td class=\"midlist\">". (($row["optimal_load"]==='y') ? $lang["optimal"] : $lang["fast"]) ."</td>";
            ...
            $rows .= "<td class=\"midlist\">". (($row["credit_free_distribution"]==='y') ? "<img src=\"layout/images/dialog-ok.png\" />" : "<img src=\"layout/images/dialog-error.png\" />") ."</td>";
            $rows .= "</tr>";
    ...
    <table class="list">
        <tr class="listtitle">
            ...
            <td class="list"><?=$sortlinks["loaded"]?></td>
            <td class="list"><?=$sortlinks["fifo_distribution"]?></td>
            <td class="list"><?=$sortlinks["credit_free_distribution"]?></td>
        </tr>
        <?=$rows?>
    </table>
