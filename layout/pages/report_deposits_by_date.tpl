...
<form name="dataform" id="dataform" enctype="multipart/form-data" method="GET" action="<?=$_SERVER["PHP_SELF"]?>">
...
if (isset($pi["note"]) && $pi["note"] != ""){
    ...
    while ($row = db_fetch_assoc($listdata)){
        echo "<form id=\"g". $row["log_depositlocations_garments_garment_id"] ."\" enctype=\"multipart/form-data\" method=\"POST\" action=\"garment_details.php\"><input type=\"hidden\" name=\"page\" value=\"details\"><input type=\"hidden\" name=\"id\" value=\"". $row['log_depositlocations_garments_garment_id'] ."\"><input type=\"hidden\" name=\"gosubmit\" value=\"true\"></form>";
        ...
        $rows .= "<td class=\"list\">". $row["log_depositlocations_garments_date"] ."</td>";
        $rows .= "<td class=\"list\">". $row["articles_description"] ."</td>";
        $rows .= "<td class=\"list\">". $row["sizes_name"] ."</td>";
        $rows .= "<td class=\"list lpointer\" onClick=\"document.getElementById('g". $row["log_depositlocations_garments_garment_id"] ."').submit();\">". $row["garments_tag"] ."</td>";
        
        if(isset($garmentusers[$row["garments_tag"].trim($row["log_depositlocations_garments_date"])])) {

            $rows .= "<td class=\"list lpointer\" onClick=\"document.getElementById('gu". $garmentusers[$row["garments_tag"].trim($row["log_depositlocations_garments_date"])] ."').submit();\">". $history[$row["garments_tag"].trim($row["log_depositlocations_garments_date"])] . "</td>";
        } else {
            $rows .= "<td class=\"list\">". $history[$row["garments_tag"].trim($row["log_depositlocations_garments_date"])] . "</td>";
        }
