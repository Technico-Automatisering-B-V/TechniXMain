...
<form name="dataform" enctype="multipart/form-data" method="GET" action="<?=$_SERVER["PHP_SELF"]?>">
    <div class="filter">
        <table>
            ...
            <? endif ?>
            <tr>
                <td class="name"><?=$lang["loaded_of"]?>:</td>
                <td class="value">
    ...
    if (isset($urlinfo["limit_total"]) && $urlinfo["limit_total"] != 0){ ?>
        ...
        <table class="list">
                <tr class="listtitle">
                    ...
                    <th class="list"><?=$sortlinks["location"]?></th>
                    <th class="list"><?=$sortlinks["station"]?></th>
                    <th class="list"><?=$sortlinks["hook"]?></th>
                    <th class="list"><?=$sortlinks["tag"]?></th>
                    <th class="list"><?=$sortlinks["article"]?></th>
                    <th class="list"><?=$sortlinks["size"]?></th>
                    <th class="list"><?=$sortlinks["owner"]?></th>
                </tr>
                ...
                <tr class="listnc">
                    ...
                    <td class="list"><?=$row["log_distributors_load_endtime"]?></td>
                    <td class="list"><?=$row["circulationgroups_name"]?></td>
                    <td class="list"><?=$row["distributors_doornumber"]?></td>
                    <td class="list"><?=$row["log_distributors_load_hook"]?></td>
                    ...
                    <td class="midlist"><?=$row["sizes_name"]?><?=(!empty($row["modifications_name"])) ? " ". $row["modifications_name"] : ""?></td>
                    <td class="list lpointer" onClick="document.getElementById('gu<?=$row["garmentusers_id"]?>').submit();">
                        ...
                        if (!empty($row["garmentusers_surname"]))
                        {
                           ...
                           print("<span class=\"empty\">" . $lang["garment_by_size"] . "</span>");
                        }
                        ?>
                    </td>
                </tr>
