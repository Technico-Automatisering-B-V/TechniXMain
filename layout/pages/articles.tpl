...
if (isset($pi["note"]) && $pi["note"] != ""){
    ...
    while ($row = db_fetch_assoc($listdata)){
        ...
        $rows .= "<tr class=\"list\" onClick=\"document.getElementById('". $row["id"] ."').submit();\"><td class=\"list\">". $row["description"] ."</td><td class=\"midlist\">". (($row["articlenumber"]) ? $row["articlenumber"] : "<font color=\"#888888\"><i>". $lang["none"] ."</i></font>") ."</td></tr>";
    } ?>

    <table class="list">
      <tr class="listtitle">
        <th class="list"><?=$sortlinks["description"]?></th>
        <th class="midlist"><?=$sortlinks["articlenumber"]?></th>
      </tr>
      <?=$rows?>
    </table>
