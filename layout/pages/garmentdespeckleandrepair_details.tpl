...
<form name="dataform" enctype="multipart/form-data" method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
	...
	<div id="tabs">
		<ul>
			<li><a href="#tab1"><?=$lang["despeckleandrepair_details"]?></a></li>
		</ul>
		<div id="tab1">
			<table class="detailstab">
				...
				<tr>
					...
					<td class="value">
						<? if ($tag_found): ?>
							<? if ($pi["page"] != "add"): ?>
							<b><?=$types[$detailsdata["type"]]?></b>
							<? else: ?>
							<? html_selectbox_array_submit("type", $types, $detailsdata["type"]) ?>
							<? endif ?>
						<? else: ?>
							&nbsp;
						<? endif ?>
				...
				<tr>
                                    <? if (isset($detailsdata["type"]) && $detailsdata["type"] == "despeckle"): ?>
                                    <td class="name">
                                            <? if ($pi["page"] != "add"): ?>
                                            <b><?=$lang["despeckle"]?>:</b>
                                            <? else: ?>
                                    ...
                                    <? endif ?>
                                    <? if (isset($detailsdata["type"]) && $detailsdata["type"] == "repair"): ?>
                                    <td class="name">
                                            <? if ($pi["page"] != "add"): ?>
                                            <b><?=$lang["repair"]?>:</b>
                                            <? else: ?>
                                    ...
                                    <td class="value">
                                            <? if ($tag_found): ?>
                                                    <? if ($pi["page"] != "add"): ?>
                                                    <b><?=$repairs[$detailsdata["repair_id"]]?></b>
                                                    <? else: ?>
	...
	&nbsp;<input type="submit" name="finalize" value="<?=(isset($detailsdata["type"]) && $detailsdata["type"] == "repair")? $lang["finalize_repair"] : $lang["finalize_despeckle"]?>" title="<?=(isset($detailsdata["type"]) && $detailsdata["type"] == "repair")? $lang["finalize_repair"] : $lang["finalize_despeckle"]?>">
	<? endif ?>
