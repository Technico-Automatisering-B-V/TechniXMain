...
<form name="dataform" enctype="multipart/form-data" method="POST" action="<?=$_SERVER["PHP_SELF"]?>">
	...
	<div id="tabs">
		<ul>
			<li><a href="#tab1"><?=$lang["replace_tag"]?></a></li>
		</ul>
		<div id="tab1">
			<table class="detailstab">
				...
				<tr>
					...
					<td class="value">
						...
						<?=$detailsdata["new_tag"]?>
						<? else: ?>
						<input type="text" id="new_tag" name="new_tag" value="<?=$detailsdata["new_tag"]?>" size="30" onKeyPress="return keep_tag(this,event)"<?=($tag_found)?"":" disabled=\"disabled\""?> /> <button class="required" title="<?=$lang["field_required"]?>">*</button>
						<? endif ?>
					</td>
				</tr>
				<tr><td class="name" colspan="2">&nbsp;</td></tr>
				<tr><td class="name"><?=$lang["location"]?>:</td><td class="value"><?=(($tag_found)?$circulationgroup:"&nbsp;")?></td></tr>
	...
	<? if ($pi["page"] == "add"): ?>
	<input type="submit" name="detailssubmit" value="<?=$lang["replace_tag"]?>" title="<?=$lang["replace_tag"]?>"<?=($tag_found)?"":" disabled=\"disabled\""?>>
	<? endif ?>
