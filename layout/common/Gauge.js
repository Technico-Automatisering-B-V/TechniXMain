(function () {
	...
	GaugeChartHelper.prototype.updateGaugeDimensions = function() {
		...
		this.arrowLength = this.chart.radiusLength;
	};
	...
	GaugeChartHelper.prototype.renderValueLabel = function() {
                if (typeof this.data.valueLabel !== 'undefined') {
                    var valueLabel = this.data.valueLabel.toString();
                } else {
                    var valueLabel = "";
                }
                var label = this.data.value.toString();
                var ctx = this.ctx;
		ctx.font = "25px " + this.fontStyle;
		var stringWidth = ctx.measureText(label).width;
		var elementWidth = 0.75 * this.gaugeRadius * 2;
		var widthRatio = elementWidth;
		var newFontSize = Math.floor(widthRatio/2.5);
		var fontSizeToUse = newFontSize;
		...
		ctx.fillText(label, this.gaugeCenterX, this.gaugeCenterY);
                ...
                var width = (ctx.measureText(label).width/2)+15;
                
                ctx.font = fontSizeToUse/3 + "px " + this.fontStyle;
		ctx.fillText(valueLabel, this.gaugeCenterX+width, this.gaugeCenterY);
	};
