 ...
 * Page settings
 */
$pi["access"] = array("reports", "found_items");
$pi["group"] = $lang["reports"];
$pi["title"] = $lang["found_items"];
$pi["filename_list"] = "found_items.php";
$pi["filename_details"] = "garmentuser_details.php";
$pi["template"] = "layout/pages/found_items.tpl";
$pi["page"] = "list";
...
if (!empty($_GET["cid"])){
    ...
    $where = " WHERE c.id = ". trim($_GET["cid"]);
} else {
    $urlinfo["cid"] = null;
    $where = " ";
}
...
if (!empty($_GET["fiid"])) {$urlinfo["fiid"] = $_GET["fiid"];} else {$urlinfo["fiid"] = "";}
...
if (!empty($_GET["col-found_item_type"])){ $urlinfo["col-found_item_type"] = $_GET["col-found_item_type"]; }else{ $urlinfo["col-found_item"] = ""; }
...
if (!empty($_GET["col-found_item_type"])){ $urlinfo["col-found_item_type"] = $_GET["col-found_item_type"]; }else{ $urlinfo["col-found_item_type"] = ""; }
...
// Required for selectbox: found_item_Type_id
$found_item_types_conditions["order_by"] = "name";
$found_item_types = db_read("found_item_types", "id value", $found_item_types_conditions);
$found_item_types_count = db_num_rows($found_item_types);
...
$query = "CREATE VIEW `". $table ."` AS (SELECT
               ...
               `f`.`value` AS 'functions_value',
                ...
                fi.found_item_type_id AS found_item_type_id,
                fit.`value` AS 'found_item_type',
                g.tag AS 'tag'
    FROM `found_items` `fi`
    INNER JOIN `garmentusers` `gu` ON `fi`.`garmentuser_id` = `gu`.`id` AND ISNULL(`gu`.`deleted_on`)
    INNER JOIN `garments` `g` ON `g`.`id` = `fi`.`garment_id`
    INNER JOIN `arsimos` `a` ON `a`.`id` = `g`.`arsimo_id`
    INNER JOIN `articles` `ar` ON `ar`.`id` = `a`.`article_id`
    INNER JOIN `sizes` `s` ON `s`.`id` = `a`.`size_id`
		INNER JOIN found_item_types fit ON fit.id = fi.found_item_type_id
    ...
    INNER JOIN `professions` `p` ON `p`.`id` = `gu`.`profession_id`
     ...
     LEFT JOIN `functions` `f` ON `f`.`id` = `gu`.`function_id`
        ".
    $where
   ." GROUP BY `gu`.`id`, fi.garment_id,fi.date
      ORDER BY `gu`.`surname`, `gu`.`name`,fi.date DESC)";
...
$columns .= " garmentusers_maidenname garmentusers_title articles_id articles_description sizes_id sizes_name sizes_position modifications_id ";
...
$columns .= " functions_id functions_value date found_item_type_id found_item_type tag";
...
if (isset($urlinfo["fiid"]) && !empty($urlinfo["fiid"])) {
    $urlinfo["where"]["8"] = "found_item_type_id = " . $urlinfo["fiid"];
}
...
$sortlinks["found_item_type"] = generate_sortlink("found_item_type", $lang["found_item_type"], $pi, $urlinfo);
...
if ($hassubmit == "export") {
    ...
    if(!empty($urlinfo["col-found_item_type"]))$header.=$lang["found_item_type"]."\t";
    ...
    while(!empty($listdata) && $row = db_fetch_array($listdata)) {
        ...
        if(!empty($urlinfo["col-found_item_type"]))array_push($in, $row["found_item_type"]);
...
$cv = array(
    ...
    "found_item_types_count" => $found_item_types_count
);
