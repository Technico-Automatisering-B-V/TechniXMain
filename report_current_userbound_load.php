 ...
 * Page settings
 */
$pi["access"] = array("load", "current_userbound_load");
$pi["group"] = $lang["load"];
$pi["title"] = $lang["current_userbound_load"];
$pi["filename_list"] = "report_current_userbound_load.php";
$pi["filename_details"] = "report_current_userbound_load.php";
$pi["template"] = "layout/pages/report_current_userbound_load.tpl";
$pi["page"] = "simple";
$urlinfo = "";
...
if (!empty($circulationgroup_id)) {
    ...
    if (isset($distributor_id)) {
        $distributor_where = " WHERE distributors.id = ". $distributor_id;
    }
    // Medewerkers
    if ($emplocle == "employees") {
        if ($status == "deleted") {
            $delWhere = isset($distributor_id)?" AND garmentusers.deleted_on IS NOT NULL": " WHERE garmentusers.deleted_on IS NOT NULL";
        } else {
            $delWhere = isset($distributor_id)?" AND garmentusers.deleted_on IS NULL": " WHERE garmentusers.deleted_on IS NULL";
        }
        ...
        if ($result_count == 0) {
            ...
            while ($users_data = db_fetch_assoc($users_sql)) {
                ...
                $tablerows .= "<tr class=\"list\" onClick=\"document.getElementById('". $users_data["user_id"] ."').submit();\">
                                    ...
                                    <td class=\"list\">". $users_data["description"] ."</td>
                                    <td class=\"midlist\">". $users_data["size"] ."</td>
    ...
    if ($emplocle == "articles") {
        if ($status == "deleted") {
            $delWhere = isset($distributor_id)?" AND garments.deleted_on IS NOT NULL":" WHERE garments.deleted_on IS NOT NULL";
        } else {
            $delWhere = isset($distributor_id)?" AND garments.deleted_on IS NULL":" WHERE garments.deleted_on IS NULL";
        }
