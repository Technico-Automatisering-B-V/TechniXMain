 ...
 * Page settings
 */
$pi["access"] = array("reports", "extra_dirty_garment");
$pi["group"] = $lang["reports"];
$pi["title"] = $lang["extra_dirty_garment"];
$pi["template"] = "layout/pages/report_extra_dirty_garments.tpl";
$pi["page"] = "list";
$pi["filename_this"] = "report_extra_dirty_garments.php";
$pi["filename_list"] = "report_extra_dirty_garments.php";
...
if (!empty($_GET["hassubmit"]) && empty($pi["note"])) {
    $table = "extra_dirty_garments";
    $columns = "extra_dirty_garments.date extra_dirty_garments.garment_id extra_dirty_garments.depositlocation_id";
    $columns .= " articles.description sizes.name modifications.name garments.tag";
    $columns .= " depositlocations.name";

    $urlinfo["inner_join"]["1"] = "garments extra_dirty_garments.garment_id garments.id";
    ...
    $urlinfo["inner_join"]["5"] = "depositlocations extra_dirty_garments.depositlocation_id depositlocations.id";
    ...
    if (!empty($urlinfo["lotsadays"])) {
        ...
        $urlinfo["where"]["1"] = "extra_dirty_garments.date >= ". $from_date_db;
        $urlinfo["where"]["2"] = "extra_dirty_garments.date <= ". $to_date_db;
    } else {
        $from_date_db = str_replace("-", "", $urlinfo["from_date"]) ."000000";
    ...
    if (!empty($urlinfo["did"])) {
        $urlinfo["where"]["4"] = "extra_dirty_garments.depositlocation_id = " . $urlinfo["did"];
    }
    ...
    $sortlinks["date"] = generate_sortlink("extra_dirty_garments.date", $lang["date"], $pi, $urlinfo);
    ...
    while ($ld = db_fetch_assoc($ldata)) {
        $history_query = "SELECT `last_distributions`.`starttime` AS 'log_garmentusers_garments_starttime',
            ...
            (
                SELECT `log_garmentusers_garments`.*
                 ...
                 WHERE `log_garmentusers_garments`.`garment_id` = ". $ld["extra_dirty_garments_garment_id"] ."
        ...
        if (trim($name) !== ",") {
            $history[$ld["garments_tag"]] = $name;
            $garmentusers[$ld["garments_tag"]] = $history_temp["id"];
        } else {
    ...
    if ($hassubmit == "export") {
        ...
        while ($row = db_fetch_array($listdata)) {
            ...
            $in = array(
                $row["extra_dirty_garments_date"],
                $row["articles_description"],
                $row["sizes_name"],
                $row["garments_tag"],
                ($history[$row["garments_tag"]]!=$last_used_unknown)?$history[$row["garments_tag"]]:$lang["unknown"],
