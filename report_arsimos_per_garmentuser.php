...
/**
 * Report arsimos per garmentuser
 *
 ...
 * Page settings
 */
$pi["access"] = array("reports", "garmentprofiles");
$pi["group"] = $lang["reports"];
$pi["title"] = $lang["garmentprofiles"];
$pi["filename_list"] = "report_arsimos_per_garmentuser.php";
$pi["filename_details"] = "garmentuser_details.php";
$pi["template"] = "layout/pages/report_arsimos_per_garmentuser.tpl";
$pi["page"] = "list";
...
$table = "tmp_arsimos_per_garmentuser";
...
if (!empty($_GET["col-credit"])){ $urlinfo["col-credit"] = $_GET["col-credit"]; }else{ $urlinfo["col-credit"] = ""; }
...
$query = "CREATE VIEW `". $table ."` AS (SELECT
               ...
               `gu`.`lockernumber` AS 'garmentusers_lockernumber',
               `ga`.`max_credit` AS 'credit',
    ...
    INNER JOIN `garmentusers_arsimos` `ga` ON `ga`.`garmentuser_id` = `gu`.`id`
    INNER JOIN `arsimos` `a` ON `a`.`id` = `ga`.`arsimo_id`
    ...
    INNER JOIN `sizes` `s` ON `s`.`id` = `a`.`size_id`
     ...
     LEFT JOIN `functions` `f` ON `f`.`id` = `gu`.`function_id`
         WHERE ISNULL(`gu`.`deleted_on`) AND `ga`.`userbound` = 0
    ".
    $where
    ." GROUP BY `gu`.`id`, `ga`.`arsimo_id`
       ORDER BY `gu`.`surname` ASC)";
 ...
 */
$columns = "garmentusers_surname garmentusers_name garmentusers_initials garmentusers_personnelcode garmentusers_id garmentusers_gender garmentusers_intermediate credit ";
$columns .= " garmentusers_maidenname garmentusers_title garmentusers_lockernumber articles_id articles_description sizes_id sizes_name sizes_position modifications_id modifications_name";
$columns .= " circulationgroups_id professions_id professions_name clientdepartments_id clientdepartments_name costplaces_id costplaces_value functions_id functions_value";
...
$sortlinks["credit"] = generate_sortlink("credit", $lang["credit"], $pi, $urlinfo);
...
if ($hassubmit == "export") {
    ...
    if(!empty($urlinfo["col-credit"]))$header.=$lang["credit"]."\t";
    ...
    while(!empty($listdata) && $row = db_fetch_array($listdata)) {
        ...
        if(!empty($urlinfo["col-credit"]))array_push($in, $row["credit"]);
