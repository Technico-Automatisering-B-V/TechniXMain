...
if (isset($_POST["page"]) && $_POST["page"] == "add") {
    if (isset($_POST["detailssubmit"])) {
        ...
        if (empty($detailsdata["type"])) { array_push($requiredfields, $lang["type"]); }
        else if ($detailsdata["type"] == 'despeckle' && !is_numeric($detailsdata["despeckle_id"])) { array_push($requiredfields, $lang["despeckle"]); }
        else if ($detailsdata["type"] == 'repair' && !is_numeric($detailsdata["repair_id"])) { array_push($requiredfields, $lang["repair"]); }

        $garment_status_sql = "SELECT COUNT(*)
                            FROM garments_despecklesandrepairs gd 
                            WHERE gd.date_out IS NULL
                            AND gd.garment_id = " . $garment_id;
        $garment_status = db_fetch_num(db_query($garment_status_sql));
        $garment_status = $garment_status[0];

        if(isset($garment_status) && $garment_status == 0) {
            if ($tag_found && empty($requiredfields)) {
                ...
                $insert_data["type"] = $detailsdata["type"];
                $insert_data["date_in"] = "NOW()";
                $insert_data["status"] = 1;
                $insert_data["halt"] = $detailsdata["halt"];

                //insert the given garmentdespeckle
                db_insert("garments_despecklesandrepairs", $insert_data);

                $scanlocation_id_sql = "SELECT `scanlocations`.`id`
                                    FROM `scanlocations`
                                    INNER JOIN `scanlocationstatuses` ON `scanlocations`.`scanlocationstatus_id` = `scanlocationstatuses`.`id`
                                    WHERE `scanlocationstatuses`.`name` = '". $detailsdata["type"] ."'";

                $scanlocation_id_r = db_fetch_row(db_query($scanlocation_id_sql));
                $scanlocation_id   = $scanlocation_id_r[0];

                //update the garment status (scanlocation_id)
                $garment_update_data = array("scanlocation_id" => $scanlocation_id, "sub_scanlocation_id" => null);
    ...
    if (isset($_POST["finalize"])) {
        if (isset($_POST["finalize"])) {
            ...
            db_update("garments_despecklesandrepairs", $_POST["id"], $update_data);
    ...
    $detailsdata = db_fetch_assoc(db_read_row_by_id("garments_despecklesandrepairs", $_POST["id"]));
    $tag_found = true;
...
if ($tag_found) {
    $garment_columns = "garments.tag garments.tag2 articles.description sizes.name modifications.name garments.washcount";
